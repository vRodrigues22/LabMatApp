...
// Ela possui um método abstrato `enviarNotificacao`, que deve ser implementado pelas classes que implementarem esta interface.
// O método serve para enviar notificações personalizadas de tarefas que necessitam de alertas.
interface Notificacao {
...
// As subclasses podem sobrescrever o método `getTipoTarefa` para fornecer informações adicionais sobre o tipo de tarefa.
class Tarefa {
    ...
    // Método privado que valida se a prioridade está entre os limites permitidos.
    ...
    // Método protegido que pode ser sobrescrito pelas subclasses para indicar o tipo da tarefa.
    ...
    // Método que retorna uma representação textual da tarefa, incluindo sua descrição, prioridade e categoria.
...
// O método `getTipoTarefa` é sobrescrito para indicar que a tarefa é urgente.
// Também implementa o método `enviarNotificacao`, que é responsável por enviar uma notificação informando que a tarefa é urgente.
class TarefaUrgente extends Tarefa implements Notificacao {
    ...
    // Sobrescreve o método `getTipoTarefa` para retornar "(Urgente)", indicando que essa é uma tarefa urgente.
    @Override
    protected String getTipoTarefa() {
        return " (Urgente)";
    }

    // Implementação do método `enviarNotificacao` da interface `Notificacao`.
    // Este método imprime uma mensagem alertando que a tarefa urgente precisa de atenção imediata.
    @Override
...
// O método `getTipoTarefa` é sobrescrito para indicar que a tarefa tem um prazo flexível.
class TarefaComPrazoFlexivel extends Tarefa {
    ...
    // Sobrescreve o método `getTipoTarefa` para retornar "(Prazo Flexível)", indicando que a tarefa não tem um prazo fixo.
    @Override
    protected String getTipoTarefa() {
